...
linking them into its sandboxing core. An integration adapter proves the shape
...
flowchart LR
    ...
    Sources --> Adapters[Integration adapters]
    Parent[Trusted Sandy parent] --> Adapters
    Adapters --> Contributions[Runtime-control contributions]
    Contributions --> Plan[Composition and integrity closure]
    Plan --> Core[sandy-core validation]
...
The parent performs discovery before the sandbox exists. Each adapter owns one
...
protections, and exact local endpoints. The runtime-control plan combines those
contributions once and pins every protected resource through its enclosing
...
- It lets multiple controls coexist without allowing one adapter to silently
  broaden or overwrite another adapter's policy.
...
- It avoids a dynamic plugin loader in the trusted launch path. Supported
  adapters are compiled, reviewed code with bounded inputs and explicit tests.
- It makes every policy loosening visible as a named capability with renderer
  and adjacent-negative coverage.

This design is deliberately not an arbitrary service framework. A new adapter
