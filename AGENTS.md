...
The version `0.1.x` CLI supports macOS and Linux, one foreground `run` mode,
Claude Code, Codex, OpenCode, and generic profiles, explicit filesystem and
executable grants, network allow/block, dry-run output, and optional
self-serve integrations. Exact local-host TCP exceptions and integration setup
remain macOS-only. Setup is not part of sandbox launch.
